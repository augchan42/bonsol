use {
    bitoracle_iching_callback::{CallbackInstruction, HexagramData},
...
fn main() {
    ...
    let account_exists = client.get_account(&storage_pubkey).is_ok();
    if account_exists {
        println!("Storage account {} already exists. Please use a different address or delete the existing account.", storage_pubkey);
        std::process::exit(1);
    }
